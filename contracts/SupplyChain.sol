...
contract SupplyChain {
  ...
  */
  ...
  modifier checkValue(uint _sku) {
    ...
    if (amountToRefund > 0) { // to shortcircuit and reduce gas usage
    ...
    forSale(_sku) paidEnough(items[_sku].price) checkValue(_sku)
  {
    ...
    address adrSeller = item.seller;
    address adrBuyer = msg.sender;
    item.buyer = adrBuyer;
    adrSeller.transfer(msg.value);
